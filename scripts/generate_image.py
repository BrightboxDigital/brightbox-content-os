...
def main():
    ...
    ap.add_argument("--post-id", type=int, required=True, help="WordPress post ID")
    ap.add_argument("--rerun", action="store_true", help="regenerate even if a manifest exists")
    ap.add_argument("--source", help="use this image file as the master instead of calling OpenAI")
    ap.add_argument("--focal", help="override the brief focal point (center/left/right/top/...)")
    ...
    # A supplied source image is an explicit intent to use a new image, so it always runs.
    if manifest_path.exists() and not args.rerun and not args.source:
    ...
    # 2. master: either a supplied image, or generated by OpenAI
    manifest["stage"] = "generate"; save_manifest()
    master = outdir / f"{slug}-master.png"
    if args.source:
        src = Path(args.source).expanduser()
        if not src.exists():
            die(f"source image not found: {src}", stage="source")
        subprocess.run(["magick", str(src), str(master)], check=True, capture_output=True)
        manifest["source"] = str(src)
        log(f"using supplied source image: {src.name}")
    else:
        prompt = brief.get("prompt")
        if not prompt:
    ...
    focal = (args.focal or brief.get("focal_point") or "center").lower()
    gravity = GRAVITY.get(focal, "Center")
    deriv_files = {}
